...
#
# Training script for Hybrid ControlNet restoration approach.
# Based on script/controlnet_restoration/train.py with key differences:
# - Uses MarigoldHybridControlNetRestorationPipeline (8ch UNet + ControlNet)
# - Uses MarigoldHybridControlNetRestorationTrainer
# - Reads base_checkpoint_path from config and passes to trainer
# - The trainer loads pre-trained 8ch UNet weights from base checkpoint
# --------------------------------------------------------------------------
...
if "__main__" == __name__:
    ...
    parser = argparse.ArgumentParser(
        description="Marigold : Hybrid ControlNet Image Restoration : Training"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="config/train_marigold_hybrid_controlnet_restoration.yaml",
    ...
    # -------------------- Model --------------------
    # Same as ControlNet train.py: load SD2 components individually.
    # ControlNet stays float32 (trainable), UNet/VAE/text_encoder in float16 (frozen).
    # The trainer will handle replacing UNet conv_in to 8ch and loading base checkpoint weights.
    ...
    logging.info("MarigoldHybridControlNetRestorationPipeline instantiated")

    # -------------------- Base checkpoint path --------------------
    # Hybrid-specific: read base_checkpoint_path from config.
    # This is the path to a pre-trained restoration checkpoint with 8ch UNet weights.
    # The trainer will load these weights into the UNet after replacing conv_in.
    base_checkpoint_path = cfg.base_checkpoint_path
    logging.info(f"Base checkpoint path (8ch UNet weights): {base_checkpoint_path}")
    ...
    # Hybrid-specific: pass base_checkpoint_path to trainer constructor.
    # Verified: MarigoldHybridControlNetRestorationTrainer.__init__ signature
    # requires base_checkpoint_path: str parameter.
