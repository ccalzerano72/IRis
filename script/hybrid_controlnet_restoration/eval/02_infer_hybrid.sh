#!/usr/bin/env bash
# Hybrid ControlNet Restoration Inference for Evaluation
# Runs Hybrid ControlNet restoration on test images for comparison
# Duplicated from script/controlnet_restoration/eval/02_infer_controlnet.sh
# Key differences:
# - Calls script/hybrid_controlnet_restoration/run.py
# - Uses "hybrid_" prefix instead of "controlnet_"
# - Auto-detects architecture:
#   - hybrid-003: hybrid_003_config.json present → uses --checkpoint (single path)
#   - hybrid-002: hybrid_config.json present → uses --base_checkpoint + --controlnet_checkpoint
...
denoise_steps=${5:-10}
ensemble_size=${6:-1}
ckpt=${7:-"checkpoints/hybrid-restoration-latest"}
...
# Use the original path (not realpath) to avoid resolving symlinks.
...
# Auto-detect hybrid architecture
# hybrid-003: has hybrid_003_config.json, both unet/ and controlnet/ in same checkpoint
# hybrid-002: has hybrid_config.json with base_checkpoint_path for separate UNet
...
if [ -f "${HYBRID_003_CONFIG}" ]; then
    ...
    echo "ERROR: No hybrid config found at ${CKPT_ABS_PATH}"
    echo "Expected hybrid_003_config.json or hybrid_config.json"
    exit 1
fi
...
OUTPUT_DIR="output/${subfolder}/hybrid_${CKPT_PARENT_DIR}/prediction_${processing_res}_s$(printf "%02d" ${denoise_steps})_e$(printf "%02d" ${ensemble_size})_${scheduler}${cfg_suffix}"
mkdir -p "${OUTPUT_DIR}"
...
OUTPUT_SUBDIR="hybrid_${CKPT_PARENT_DIR}_prediction_${processing_res}_s$(printf "%02d" ${denoise_steps})_e$(printf "%02d" ${ensemble_size})_${scheduler}${cfg_suffix}"
...
echo "Running Hybrid ControlNet inference..."
...
if [ ${EXISTING_COUNT} -eq ${TOTAL_COUNT} ] && [ ${TOTAL_COUNT} -gt 0 ]; then
    ...
    # Run inference using Hybrid ControlNet run.py
    # Auto-detect: 003 uses --checkpoint, 002 uses --base_checkpoint + --controlnet_checkpoint
    ...
    echo "Hybrid ControlNet inference complete. Results saved to: ${OUTPUT_ABS_DIR}"
fi
