#!/usr/bin/env bash
# Marigold Restoration Inference for Evaluation
# Runs Marigold restoration on test images for comparison
...
clean_dir=${3:-"${BASE_DATA_DIR}/restoration_test/clean"}  # NEW PARAMETER
processing_res=${4:-0}    # Processing resolution: 0=original, 512, 768, 1024
denoise_steps=${5:-10}
ensemble_size=${6:-1}
ckpt=${7:-"checkpoints/marigold-restoration-latest"}
scheduler=${8:-"ddim"}    # Scheduler type: ddim, lcm, heun
guidance_scale=${9:-1.0}  # CFG scale: 1.0=no CFG, >1.0=apply CFG
use_cpu=${10:-""}         # Set to "cpu" to force CPU inference
max_images=${11:-""}      # Max images to process per run (empty = all)
...
OUTPUT_DIR="output/${subfolder}/marigold_${CKPT_PARENT_DIR}/prediction_${processing_res}_s$(printf "%02d" ${denoise_steps})_e$(printf "%02d" ${ensemble_size})_${scheduler}${cfg_suffix}"
mkdir -p "${OUTPUT_DIR}"
...
OUTPUT_SUBDIR="marigold_${CKPT_PARENT_DIR}_prediction_${processing_res}_s$(printf "%02d" ${denoise_steps})_e$(printf "%02d" ${ensemble_size})_${scheduler}${cfg_suffix}"
...
echo "Running Marigold inference..."
...
if [ ${EXISTING_COUNT} -eq ${TOTAL_COUNT} ] && [ ${TOTAL_COUNT} -gt 0 ]; then
    ...
    # Build command with optional --cpu flag
    CPU_FLAG=""
    CUDA_PREFIX=""
    if [ "${use_cpu}" = "cpu" ]; then
        CPU_FLAG="--cpu"
    ...
    echo "Marigold inference complete. Results saved to: ${OUTPUT_ABS_DIR}"
fi
