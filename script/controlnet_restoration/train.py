...
#
# Training script for ControlNet restoration approach.
# Based on script/restoration/train.py but adapted for ControlNet architecture:
# - Loads SD2 components individually
# - Creates ControlNet from UNet via ControlNetModel.from_unet()
# - ControlNet stays float32 (trainable), UNet/VAE/text_encoder in float16 (frozen)
# - Uses MarigoldControlNetRestorationTrainer
# --------------------------------------------------------------------------
...
if "__main__" == __name__:
    ...
    parser = argparse.ArgumentParser(
        description="Marigold : ControlNet Image Restoration : Training"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="config/train_marigold_controlnet_restoration.yaml",
    ...
    # -------------------- Model --------------------
    # Key difference from restoration/train.py:
    # We load SD2 components individually and create ControlNet from UNet.
    # ControlNet stays float32 (trainable params need float32 for GradScaler).
    # UNet, VAE, text_encoder stay float16 (frozen, saves VRAM).
    ...
    logging.info("MarigoldControlNetRestorationPipeline instantiated")
