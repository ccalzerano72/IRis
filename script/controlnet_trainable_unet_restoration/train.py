# --------------------------------------------------------------------------
# Thesis Implementation: 4ch Trainable UNet + ControlNet (Ablation re_004)
# Blind Image Restoration
#
# Training script for the 4ch trainable UNet + ControlNet ablation experiment.
# Based on script/hybrid_controlnet_restoration_003/train.py with key differences:
# - Uses MarigoldControlNetRestorationPipeline (4ch UNet, no latent concat)
# - Uses MarigoldControlNetTrainableUnetTrainer
# - UNet loaded in float32 (trainable)
# - No ARNIQA, no 8ch expansion, no degraded latent concatenation
# --------------------------------------------------------------------------
...
from src.trainer.marigold_controlnet_trainable_unet_trainer import (
    MarigoldControlNetTrainableUnetTrainer,
)
...
if "__main__" == __name__:
    ...
    parser = argparse.ArgumentParser(
        description="Marigold : 4ch Trainable UNet + ControlNet Image Restoration : Training"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="config/train_marigold_controlnet_trainable_unet_restoration.yaml",
    ...
    # -------------------- Model --------------------
    # UNet in float32 — TRAINABLE (same as hybrid-003 train script)
    # ControlNet in float32 — TRAINABLE
    # VAE and text_encoder in float16 — FROZEN
    ...
    logging.info("MarigoldControlNetRestorationPipeline instantiated (4ch UNet, trainable)")
    ...
    trainer = MarigoldControlNetTrainableUnetTrainer(
        cfg=cfg,
        model=model,
