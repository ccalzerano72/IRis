...
# PROJECT_ROOT for dither extension (optional, not critical)
from pathlib import Path
PROJECT_ROOT = Path(__file__).absolute().parents[1].absolute()

# Dither extension - only available on Linux/Unix systems
try:
    dither_path = str(PROJECT_ROOT / "utils" / "dither_extension/dither.so")
    dither_cpp = ctypes.CDLL(dither_path).dither
    ...
    # Dither extension not available (Windows or missing library)
...
def impulse_noise(x: torch.Tensor, d: float, s_vs_p: float = 0.5) -> torch.Tensor:
    y = x.clone()  # Avoid modifying input tensor
    
    # Fix: Calculate number of pixels to corrupt in spatial dimensions only
    # Original bug: multiplied by all 3 dimensions creating too many coordinates
    C, H, W = y.shape
    num_sp = int(d * H * W)  # Only spatial pixels, not channels
    
    if num_sp == 0:
        return y
    
    # Generate random spatial coordinates using torch (device-aware)
    h_coords = torch.randint(0, H, (num_sp,), device=y.device)
    w_coords = torch.randint(0, W, (num_sp,), device=y.device)
    
    # Split into salt and pepper
    num_salt = int(s_vs_p * num_sp)
    
    # Apply salt (white = 1) and pepper (black = 0) to all channels
    y[:, h_coords[:num_salt], w_coords[:num_salt]] = 1.0
    y[:, h_coords[num_salt:], w_coords[num_salt:]] = 0.0
