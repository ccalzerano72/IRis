...
wget https://github.com/ccalzerano72/IRis/releases/download/v1.0.0/002_re_015000.z01
wget https://github.com/ccalzerano72/IRis/releases/download/v1.0.0/002_re_015000.z02
wget https://github.com/ccalzerano72/IRis/releases/download/v1.0.0/002_re_015000.z03
wget https://github.com/ccalzerano72/IRis/releases/download/v1.0.0/002_re_015000.z04
wget https://github.com/ccalzerano72/IRis/releases/download/v1.0.0/002_re_015000.zip

# Extract — choose one method:

# Option A: unzip (Linux/Mac, requires unzip >= 6.0 with AES support)
cat 002_re_015000.zip 002_re_015000.z01 002_re_015000.z02 002_re_015000.z03 002_re_015000.z04 > combined.zip
unzip combined.zip -d checkpoints/

# Option B: 7-Zip standalone (recommended, no root required)
wget https://www.7-zip.org/a/7z2602-linux-x64.tar.xz
tar xf 7z2602-linux-x64.tar.xz
./7zz x 002_re_015000.zip -o./checkpoints/

# Option C: Windows (7-Zip GUI)
...
```bash
chmod +x infer.sh
./infer.sh path/to/degraded/images path/to/output
```

For full control over all parameters, call `run.py` directly:
...
> **Note on base model**: the model was trained on `stabilityai/stable-diffusion-2`, which has since been removed from HuggingFace. The default falls back to `sd2-community/stable-diffusion-2-1`, a community mirror. SD 2.1 shares identical VAE and text encoder components with SD 2.0 and produces equivalent inference results. To use a local copy instead: `--base_model /path/to/local/sd2`.
